...
def add_nutrient_values(dish_nutrient, total_nutrient):
    ...
    st.session_state.total_nutrient_values.append(total_nutrient)
    st.session_state.dish_nutrient_values.append(dish_nutrient)

def detect_image_result(detected_image, resized_uploaded_image):
    # st.write(detected_image)
    # st.write(type(detected_image))
    # boxes = detected_image[0].boxes

    if detected_image["predictions"]:

        predictions = detected_image.get("predictions", [])

        image_width = detected_image["image"]["width"]
...
def detect_image(conf, uploaded_file, model, url=False):
        ...
        if st.session_state.button_clicked and not reset_button:
            with st.spinner("Running..."):
                ...
                # img_buffer = BytesIO()
                if resized_uploaded_image.mode == "RGBA":
                    resized_uploaded_image = resized_uploaded_image.convert("RGB")

                with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as temp_file:
                    resized_uploaded_image.save(temp_file, format="JPEG")
...
class VideoTransformer(VideoTransformerBase):
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        for r in results:
            im_bgr = r.plot()
            print("img_bgr shape: ", im_bgr.shape)
            for pred in r.boxes:
                class_id = int(pred.cls[0].item())
                if isinstance(pred.xyxy, torch.Tensor):
                    boxes = pred.xyxy.cpu().numpy()
                ...
                # bbox_coordinates = (x2/640 + 5, y1 / 640)
                x1, y1, x2, y2 = x1 * w / 640, y1 * h / 480, x2 * w / 640, y2 * h / 480
                x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)

                print(f"x1: {x1}, y1: {y1}, x2: {x2}, y2: {y2}") 
                # bbox_coordinates = (x2 + 40, y1 - 800)
                bbox_coordinates = (x2, y1)
                # bbox_coordinates = (x_center, y_center)
            ...
            # fat_color, fat_desc = get_traffic_light_color("Fat", nutrition.get('Fat'), serving)
            # sugar_color, sugar_desc = get_traffic_light_color("Sugar", nutrition.get('Sugar'), serving)
            # saturates_color, saturates_desc = get_traffic_light_color("Saturates", nutrition.get('Saturates'), serving)
            # salt_color, salt_desc = get_traffic_light_color("Salt", nutrition.get('Salt'), serving)

            # draw_traffic_light_bars(im_bgr, x2, y1, fat_color = fat_color, sugar_color = sugar_color, saturates_color = saturates_color, salt_color = salt_color)
    ...
    if webrtc_ctx.state.playing:
        ...
        while True:
            ...
            for detection in detections:
                ...
                frame = detection.the_frame
                x2, y1 = detection.bbox_coordinates
...
<div class="each-nutri-container">
    ...
    <div class="each-nutri-box" style="background-color: {salt_color};">
        ...
        <span class="each-nutri-percentage">{percentage_contribution['Salt']:.1f}%</span>
                            ...
                            """
#                         nutrition_html = f"""
# <div style="position:absolute; left:{int(x_pos) + 5}px; top:{int(y_pos)}px; padding:5px; background:rgba(255,255,255,0.8); border-radius:5px;">
#     <p>Calories: {nutrition.get('Calories')} kcal</p>
#     <p style="color:{fat_color};">Fat: {nutrition.get('Fat')}g</p>
#     <p style="color:{saturates_color};">Saturates: {nutrition.get('Saturates')}g</p>
#     <p style="color:{sugar_color};">Sugar: {nutrition.get('Sugar')}g</p>
#     <p style="color:{salt_color};">Salt: {nutrition.get('Salt')}g</p>
#     <p>X: {x_pos}, Y: {y_pos}, Type: {type(x_pos)}</p>
# </div>
# """
                    ...
                    <hr style="border: none; border-top: 1px dashed black; width: 80%;">
                    """)
                        # detection_results += nutrition_html
                        for key in total_nutrition:
                                    if key in nutrition:
                                        total_nutrition[key] += nutrition[key]
